    ...
    return (<div className="list-receipt-table-container">
        ...
        <table class="table table-hover table-customize">
            ...
            <tbody>
                ...
                // </tr>
                {
                 return   arrayHeaders.map(header => {
                    
                       if(header.includes("input")){
                           return  <td><input  className="input-table" disabled={props.disabled} name={header[0]} onChange={props.onChangeValueProduct} value={Object.entries(product)[Object.entries(product).map(item => item[0]).indexOf(header[0])][1]} /> </td>
                       }
                       else{
