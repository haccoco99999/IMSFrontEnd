...
export default function SearchComponent(props){
   ...
   function onblur(){
    ...
    return(
        ...
        <input name="keySearch" onChange={changeInput} type="text" class="form-control" placeholder="Search by Order ID or Supplier Name" />
        {listSearchProduct.length !== 0 ? <ul className="list-search">
            {listSearchProduct.map((product, index) => {
                return (
                    ...
                    <h4 onClick={() => setListSearchProduct([])} onMouseUp={() => props.clickToAddProduct(product)} className="text-add-product">+Add</h4></li>
